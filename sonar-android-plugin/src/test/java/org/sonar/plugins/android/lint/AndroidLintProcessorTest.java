...
public class AndroidLintProcessorTest {
  ...
  public void process_empty_report() throws Exception {
    // Process report
    try {
      new AndroidLintProcessor(context, rulesProfile).process(new File("src/test/resources/lint-report-empty.xml"));
    } catch (Exception e) {
      Fail.fail();
    }
  ...
  public void process_report_with_relative_path() throws Exception {
    // Process report
    new AndroidLintProcessor(context, rulesProfile).process(new File("src/test/resources/lint-report.xml"));
...
//    verify(perspectives, times(30)).as(any(Class.class), any(InputPath.class));
  ...
  public void process_report_with_absolute_path() throws Exception {
    // Process report
    new AndroidLintProcessor(context, rulesProfile).process(new File("src/test/resources/lint-results_absolute_path.xml"));
...
//    verify(perspectives, times(8)).as(any(Class.class), any(InputPath.class));
  ...
  public void should_handle_bad_xml_results() throws Exception {
    new AndroidLintProcessor(context, rulesProfile).process(new File("src/test/resources/lint-bad-report.xml"));
    verify(rulesProfile, never()).getActiveRule(anyString(), anyString());
...
//    verify(issuable, times(30)).addIssue(any(Issue.class));
  ...
  public void unknown_issue_should_not_be_reported() throws Exception {
    ...
    new AndroidLintProcessor(context, rulesProfile).process(new File("src/test/resources/lint-unknown-rule-report.xml"));
